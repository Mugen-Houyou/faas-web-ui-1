...
프로그램이 제공된 입력보다 더 많은 데이터를 읽으려 하면, 백엔드는 모든 입력을 전달한 뒤 표준 입력 스트림을 닫습니다. 실행 중인 프로세스는 EOF 신호를 받게 되어 더 이상 입력을 기다릴 수 없으며, 필요한 경우 오류를 발생시키거나 종료합니다. `timeLimit` 설정도 그대로 적용됩니다.
