...
- 백엔드(`online_judge_backend` 폴더)는 FastAPI 기반 `/execute_v4`, `/execute_v4_public` 엔드포인트를 제공하며 RabbitMQ로 작업을 워커에 전달합니다. `/execute`, `/execute_v2`, `/execute_v3`은 디버그용/deprecated로, 사용하지 않습니다.
...
6. `.env.example`의 양식과 동일하게 `.env` 파일을 생성하여 환경 변수를 구성합니다.
   ...
   - 참고로 RabbitMQ 서버의 기본 주소는 `amqp://guest:guest@localhost/`입니다.
...
```
이후 `http://localhost:8080/index_v4.html`에 접속하면 됩니다.
